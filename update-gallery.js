...
function generateGalleryPageHtml(images, pageNumber, totalPages) {
  const title = pageNumber === 1 ? 'PORTFOLIO' : `PORTFOLIO - 第${pageNumber}页`;
  ...
  if (totalPages > 1) {
    ...
    <div class="flex justify-center items-center space-x-4 mt-8 mb-8">
      ${pageNumber > 1 ? `<a href="${pageNumber === 2 ? '../index.html' : `gallery_page_${pageNumber - 1}.html`}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition duration-300">上一页</a>` : ''}
      <span class="text-lg font-semibold">第 ${pageNumber} 页 / 共 ${totalPages} 页</span>
      ${pageNumber < totalPages ? `<a href="gallery_page_${pageNumber + 1}.html" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition duration-300">下一页</a>` : ''}
    </div>`;
  }
...
function updateIndexHtml(images, totalPages) {
  ...
  try {
    ...
    let paginationHtml = '';
    if (totalPages > 1) {
      paginationHtml = `    <div class="flex justify-center items-center space-x-4 mt-8 mb-8">
      <span class="text-lg font-semibold">第 1 页 / 共 ${totalPages} 页</span>
      <a href="dist/gallery_page_2.html" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition duration-300">下一页</a>
    </div>`;
    }
